...
static u32 qm3x_bypass_stored_max = 2000;
module_param_named(bypass_stored_max, qm3x_bypass_stored_max, uint, 0664);
MODULE_PARM_DESC(bypass_stored_max,
		 "Maximum number of stored packets (unlimited if 0). "
		 "Default to 2000.");

/**
 * qm3x_bypass_event_cb() - Receive event transport handler callback.
 ...
 */
static void qm3x_bypass_event_cb(void *data, struct sk_buff *skb)
{
	qm3x_bypass_handle hnd = (qm3x_bypass_handle)data;
	enum qm3x_bypass_events bypass_event = QM3X_BYPASS_IRQ;

	/* Sanity check received packet type. */
	if (skb->cb[0] != hnd->expected_type) {
		goto error;
	}
	if (atomic_inc_return(&hnd->count) > qm3x_bypass_stored_max) {
		atomic_dec(&hnd->count);
		if (__ratelimit(&hnd->max_pkt_rls)) {
			struct qm3x *qm35 = container_of(
				hnd->bypass, struct qm3x, bypass_data);
			dev_warn(qm35->dev,
				 "Incoming packets queue full for UL%d. Packet "
				 "dropped.\n",
				 hnd->expected_type);
		}
		goto error;
	 ...
	 * Will in turn call qm3x_bypass_recv() which will
...
/**
 * qm3x_bypass_set_expected() - Associated bypass channel to packet type.
 * @hnd: The bypass channel handle to configure.
 * @expected: The new expected packet type to handle.
 *
 * This function is called by qm3x_bypass_open() or qm3x_bypass_control() by
 * the client (``/dev/uciX``).
 *
 * It install the qm3x_bypass_event_cb() transport handler for the specified
 ...
 */
static int qm3x_bypass_set_expected(qm3x_bypass_handle hnd,
				    enum qm3x_transport_msg_type expected)
{
	struct qm3x *qm35 = container_of(hnd->bypass, struct qm3x, bypass_data);
	...
	if (qm3x_bypass_bound(hnd, &cur)) {
		...
		qm3x_transport_unregister(qm35, cur, QM3X_TRANSPORT_PRIO_HIGH,
					  qm3x_bypass_event_cb);
		hnd->expected_type = QM3X_TRANSPORT_MSG_MAX;
	}
	...
	rc = qm3x_transport_register(qm35, expected, QM3X_TRANSPORT_PRIO_HIGH,
				     qm3x_bypass_event_cb, hnd);
	if (!rc)
...
/**
 * qm3x_bypass_cleanup() - Cleanup a bypass channel.
 * @hnd: The bypass channel handle to configure.
 *
 ...
 *
 * This function also ensures the qm3x_bypass_event_cb() transport handler for
 * the expected packet type is unregistered correctly.
 *
 * Return: Number of freed unread skb or zero if none.
 */
static int qm3x_bypass_cleanup(qm3x_bypass_handle hnd)
{
	struct qm3x *qm35 = container_of(hnd->bypass, struct qm3x, bypass_data);
	enum qm3x_transport_msg_type cur;
	struct sk_buff *p, *n;
	...
	if (rc)
		dev_warn(qm35->dev,
			 "Channel for UL%d closed while %d packet(s) "
			 "remain in queue!\n",
			 cur, rc);
	if (hnd->max_pkt_rls.missed)
		dev_warn(qm35->dev,
			 "Ratelimiting suppressed %d packet dropped warnings "
			 "for UL%d.\n",
			 hnd->max_pkt_rls.missed, cur);
	/* Re-enable IRQ if it was disabled by qm3x_bypass_control(). */
	if (hnd->irq_disabled)
		qm3x_transport_set_irq(qm35, 1);
	/* We already printed a custom message, avoid pr_warn() call in
	 * ratelimit_state_exit(). */
	ratelimit_set_flags(&hnd->max_pkt_rls, 0);
	ratelimit_state_exit(&hnd->max_pkt_rls);
	/* Free bypass channel. */
	kfree(hnd);
	return rc;
}
...
/**
 * qm3x_bypass_open() - Open a bypass channel.
 ...
 *
 * It also calls the ``qm3x_transport_start()`` to allow the transport module
...
{
	struct qm3x_bypass *bypass = &qm35->bypass_data;
	qm3x_bypass_handle hnd;
	int rc = -EINVAL;

	trace_qm3x_bypass_open(qm35, cb, priv_data);
	if (!qm35 || !cb) {
		goto error;
	}

	hnd = kmalloc(sizeof(struct qm3x_bypass_channel), GFP_KERNEL);
	if (!hnd) {
		rc = -ENOMEM;
		goto error;
	}
	...
	hnd->expected_type = QM3X_TRANSPORT_MSG_MAX;
	hnd->irq_disabled = false;
	atomic_set(&hnd->count, 0);
	ratelimit_state_init(&hnd->max_pkt_rls, 60 * HZ, 3);
	/* We don't want __ratelimit() to print missed message warning. */
	ratelimit_set_flags(&hnd->max_pkt_rls, RATELIMIT_MSG_ON_RELEASE);
	...
	if (atomic_inc_return(&bypass->opened) == 1) {
		...
		rc = qm3x_transport_start(qm35);
		if (rc < 0)
			goto error_put;
	}

	trace_qm3x_bypass_open_return(qm35, hnd);
	return hnd;
...
error_free:
	...
	kfree(hnd);
error:
	trace_qm3x_bypass_open_return(qm35, ERR_PTR(rc));
	return ERR_PTR(rc);
}
EXPORT_SYMBOL(qm3x_bypass_open);

/**
 * qm3x_bypass_queue_check() - Returns the status of the bypass packet queue.
...
EXPORT_SYMBOL(qm3x_bypass_queue_check);

/**
 * qm3x_bypass_close() - Close a bypass channel.
 * @hnd: The bypass channel handle.
 *
 ...
 *
 * It will also call ``qm3x_transport_stop()`` to allow the transport module to
...
int qm3x_bypass_close(qm3x_bypass_handle hnd)
{
	...
	struct qm3x *qm35 = NULL;
	int rc = -EINVAL;

	trace_qm3x_bypass_close(hnd);
	if (IS_ERR_OR_NULL(hnd))
		goto error;
	bypass = hnd->bypass;
	qm35 = container_of(bypass, struct qm3x, bypass_data);
	...
	rc = qm3x_bypass_cleanup(hnd);

	if (atomic_dec_and_test(&bypass->opened)) {
		/* Last close. No more channel opened. Ensure the QM35 chip is
		 * stopped. */
		qm3x_transport_stop(qm35);
	}
	/* Decrease module usage counter. */
	module_put(THIS_MODULE);

error:
	trace_qm3x_bypass_close_return(qm35, rc);
	return rc;
}
EXPORT_SYMBOL(qm3x_bypass_close);

/**
 * qm3x_bypass_control() - Control a bypass channel.
 * @hnd: The bypass channel handle.
 ...
 *
 * More actions may be added depending of needs. See ``enum qm3x_bypass_actions``.
 *
			...
			long *param)
{
	...
	trace_qm3x_bypass_control(hnd, action, param);
	if (IS_ERR_OR_NULL(hnd))
		goto error;
	bypass = hnd->bypass;
	qm35 = container_of(bypass, struct qm3x, bypass_data);
	...
	case QM3X_BYPASS_ACTION_RESET:
		if (IS_ERR_OR_NULL(param))
			break;
		rc = qm3x_transport_reset(qm35, *param);
		break;
	case QM3X_BYPASS_ACTION_MSG_TYPE:
		...
		if (param) {
			new = *param;
			rc = qm3x_bypass_set_expected(hnd, new);
			if (!rc)
				rc = cur;
		} else {
			rc = cur;
	...
	case QM3X_BYPASS_ACTION_FWUPD:
		rc = qm3x_transport_fw_update(qm35, NULL, 0, (char *)param);
		/* Reprobe device in case of success with firmware flashed. */
		if (rc == 2) {
			char infobuf[64];
			/* Temporarily unregister qm3x_bypass_event_cb() so
			 * qm35_uci_probe_handle() can be set up instead. */
			qm3x_transport_unregister(qm35, hnd->expected_type,
						  QM3X_TRANSPORT_PRIO_HIGH,
						  qm3x_bypass_event_cb);
			rc = qm3x_transport_probe(qm35, infobuf, sizeof(infobuf)) ?:
				     rc;
			qm3x_transport_register(qm35, hnd->expected_type,
						QM3X_TRANSPORT_PRIO_HIGH,
						qm3x_bypass_event_cb, hnd);
		}
		break;
	case QM3X_BYPASS_ACTION_IRQ:
		if (IS_ERR_OR_NULL(param))
			break;
		/* Make sure to disable only once at most, and re-enable when
		 * bypass is closed. */
		if (!*param && !hnd->irq_disabled) {
			/* Disable IRQ. */
			hnd->irq_disabled = true;
			qm3x_transport_set_irq(qm35, *param);
		} else if (*param && hnd->irq_disabled) {
			/* Enable IRQ. */
			hnd->irq_disabled = false;
			qm3x_transport_set_irq(qm35, *param);
		}
		rc = 0;
		break;
	case QM3X_BYPASS_ACTION_WAIT_IRQ:
		if (IS_ERR_OR_NULL(param))
			break;
		rc = qm3x_transport_wait_irq(qm35, *param);
		break;
	case QM3X_BYPASS_ACTION_SPI_TRANSFER:
		if (IS_ERR_OR_NULL(param))
			break;
		rc = qm3x_transport_spi_transfer(qm35,
						 (struct spi_transfer *)param);
		break;
	case QM3X_BYPASS_ACTION_POWER:
		if (IS_ERR_OR_NULL(param))
			break;
		rc = qm3x_transport_power(qm35, *param);
		break;
	default:
...
error:
	trace_qm3x_bypass_control_return(hnd, rc);
	return rc;
}
EXPORT_SYMBOL(qm3x_bypass_control);

/**
 * qm3x_bypass_send() - Send data to transport.
 * @hnd: The bypass channel handle.
...
int qm3x_bypass_send(qm3x_bypass_handle hnd, void *buffer, size_t len)
{
	...
	trace_qm3x_bypass_send(hnd);
	if (IS_ERR_OR_NULL(hnd) || !buffer || !len)
		goto error;
	if (!qm3x_bypass_bound(hnd, NULL)) {
		rc = -EBADTYPE;
		goto error;
	}
	...
	rc = qm3x_transport_send_direct(qm35, hnd->expected_type, buffer, len);
error:
	trace_qm3x_bypass_send_return(hnd, rc);
	return rc;
}
EXPORT_SYMBOL(qm3x_bypass_send);

/**
 * qm3x_bypass_recv() - Receive data from queue.
 * @hnd: The bypass channel handle.
 ...
 * saved by qm3x_bypass_event_cb() before the registered listener function is called.
 *
 * Context: User context.
...
error:
	trace_qm3x_bypass_recv_return(hnd, rc);
	return rc;
}
EXPORT_SYMBOL(qm3x_bypass_recv);
