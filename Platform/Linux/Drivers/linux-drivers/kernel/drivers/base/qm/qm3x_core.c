...
#define qm3x_notify_new(qm) qm3x_notifier_notify(qm, QM3X_NOTIFIER_EVENT_NEW)
#define qm3x_notify_online(qm) \
	qm3x_notifier_notify(qm, QM3X_NOTIFIER_EVENT_ONLINE)
#define qm3x_notify_delete(qm) \
	qm3x_notifier_notify(qm, QM3X_NOTIFIER_EVENT_DELETE)

/**
 * qm3x_state_wait() - Wait the QM35 state's change.
 * @qm35: QM35 device instance.
 * @device_state: Device state to wait for.
 *
 * Wait the QM35 state's change through a wait queue.
 * Waken up by qm3x_state_set().
 *
...
EXPORT_SYMBOL(qm3x_state_wait);

/**
 * qm3x_state_set() - Set the QM35 state.
 * @qm35: QM35 device instance.
...
{
	trace_qm3x_state(qm35, qm35->state, device_state);
	qm35->state = device_state;

	wake_up(&qm35->wait_state);
}
EXPORT_SYMBOL(qm3x_state_set);

/**
 * qm3x_parse_device_info() - Parse UCI_CORE GET_DEVICE_INFO response.
 * @qm35: QM35 device instance.
 ...
 */
static int qm3x_parse_device_info(struct qm3x *qm35,
				  struct qm3x_uci_device_info *info,
...
#undef uci_version_to_triplet
	...
	qm3x_fw_version_print(dev_info, qm35->dev, "Running firmware version",
			      current_ver);
...
/**
 * qm3x_alloc_device() - allocate a QM35 device
			       ...
			       const struct qm3x_transport *transport)
{
	...
	trace_qm3x_alloc_device_return(qm35);
	return qm35;
...
EXPORT_SYMBOL(qm3x_alloc_device);

/**
 * qm3x_free_device() - free a QM35 device
...
void qm3x_free_device(struct qm3x *qm35)
{
	...
	trace_qm3x_free_device(qm35);

	/* Remove device ID. */
	qm3x_remove_id(qm35->dev_id);
	/* Finally free */
	kfree(qm35);
}
EXPORT_SYMBOL(qm3x_free_device);

/**
 * qm3x_register_device() - register a QM35 device
...
int qm3x_register_device(struct qm3x *qm35)
{
	...
	trace_qm3x_register_device(qm35);
	if (IS_ERR_OR_NULL(qm35)) {
		rc = -EINVAL;
		goto notify_new_error;
	}
	if (qm35->transport_flags & QM3X_TRANSPORT_ASYNC_PROBING) {
	...
	rc = qm3x_transport_reset(qm35, false);
	 ...
	 * firmware either. */
	if (qm35->transport_flags & QM3X_TRANSPORT_NO_PROBING)
		goto register_hw;
	...
	do {
		...
		if (rc < 0)
			 ...
			 * or exit with error. */
			goto fw_update;
		info = (struct qm3x_uci_device_info *)infobuf;
		version_ok = !qm3x_parse_device_info(qm35, info, &current_ver,
						     &device_id);
	...
	fw_update:
		...
		if (!fw_update_tried) {
			...
			rc = qm3x_transport_fw_update(
				qm35, version_ok ? &current_ver : NULL,
				device_id, NULL);
			/* If qm3x_transport_fw_update() was interrupted by a
...
register_hw:
	...
	trace_qm3x_register_device_return(qm35, 0);
	return 0;

probe_error:
	qm3x_transport_stop(qm35);
start_error:
	qm3x_notify_delete(qm35);
notify_new_error:
	trace_qm3x_register_device_return(qm35, rc);
	return rc;
}
EXPORT_SYMBOL(qm3x_register_device);

/**
 * qm3x_unregister_device() - unregister a QM35 device
 * @qm35: the QM35 device private structure to unregister
 *
 * Unregister a QM35 device.
 *
 * Context: Called from bus device probing or module init/exit functions.
 * Return: Zero on success, else a negative error.
 */
int qm3x_unregister_device(struct qm3x *qm35)
{
	...
	trace_qm3x_unregister_device(qm35);
	if (IS_ERR_OR_NULL(qm35)) {
		rc = -EINVAL;
		goto error;
	}
...
error:
	trace_qm3x_unregister_device_return(qm35, rc);
	return rc;
}
EXPORT_SYMBOL(qm3x_unregister_device);

/**
 * qm3x_get_dev_id() - Return a QM35 device ID.
...
EXPORT_SYMBOL(qm3x_get_dev_id);

/**
 * qm3x_get_device() - Return a underlying struct device of QM35 instance.
 * @qm35: The QM35 instance.
 *
...
EXPORT_SYMBOL(qm3x_get_device);

#ifndef QM3X_CORE_TESTS

static int __init qm3x_init(void)
{
	pr_info("%s: init\n", THIS_MODULE->name);
...
static void __exit qm3x_exit(void)
{
	pr_info("%s: Exit\n", THIS_MODULE->name);
}

module_init(qm3x_init);
module_exit(qm3x_exit);
