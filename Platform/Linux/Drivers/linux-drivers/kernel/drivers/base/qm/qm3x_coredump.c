...
#include "qm3x_core.h"
#include "qm3x_coredump.h"
#include "qm3x_notifier.h"
#include "qm3x_transport.h"

static u32 qm3x_coredumps_max = 32;
module_param_named(coredumps_max, qm3x_coredumps_max, uint, 0644);
MODULE_PARM_DESC(coredumps_max,
		 "Maximum number of simultaneously saved coredumps");

static u32 qm3x_coredumps_burst = DEFAULT_RATELIMIT_BURST;
module_param_named(coredumps_burst, qm3x_coredumps_burst, uint, 0444);
 ...
 * - cmd_id, uint8_t, where values can be one from enum qm3x_coredump_cmd_id,
...
enum qm3x_coredump_status { COREDUMP_RCV_NACK, COREDUMP_RCV_ACK };
...
static void qm3x_coredumpx_remove(struct work_struct *work);
static ssize_t qm3x_coredumpx_read(struct file *filp, struct kobject *kobp,
				   struct bin_attribute *bin_attr, char *buf,
				   loff_t pos, size_t count);

static void qm3x_coredump_free_data(struct qm3x_coredump_data *cdd)
{
	list_del(&cdd->list);
	kfree(cdd);
}

static uint16_t qm3x_coredump_csum(struct qm3x_coredump_data *cdd)
{
	uint16_t csum = 0;
...
static void qm3x_coredump_send_rcv_status(struct qm3x_coredump *cd, uint8_t s)
{
	struct coredump_pkt reply;
	reply.cmd_id = COREDUMP_RCV_STATUS;
	reply.status = s;
	qm3x_transport_send(cd->qm35, QM3X_TRANSPORT_MSG_COREDUMP, &reply,
			    2); /* cannot use sizeof(reply). */
}

static void qm3x_coredump_force(struct qm3x_coredump *cd)
{
	uint8_t packet = COREDUMP_FORCE_CMD;
	qm3x_transport_send(cd->qm35, QM3X_TRANSPORT_MSG_COREDUMP, &packet,
			    sizeof(packet));
}

static struct qm3x_coredump *qm3x_coredump_search(struct qm3x *qm35)
{
	struct qm3x_coredump *cd = NULL;
	if (!qm35)
		return NULL;
...
static void qm3x_coredump_check(struct work_struct *work)
{
	struct qm3x_coredump *cd =
		container_of(work, struct qm3x_coredump, dwork.work);
	struct device *dev = cd->dev;
	struct qm3x_coredump_data *cdd;
	...
	cdd = list_first_entry(&cd->dump_list, struct qm3x_coredump_data, list);
	spin_unlock(&cd->dump_lock);
	if (cdd->remain) {
		...
		qm3x_coredump_send_rcv_status(cd,
					      cdd->status & COREDUMP_RCV_ACK);
	}
}

static int qm3x_coredump_handle_header(struct qm3x_coredump *cd,
				       struct sk_buff *skb)
{
	struct device *dev = cd->dev;
	struct qm3x_coredump_data *cdd;
...
static int qm3x_coredump_handle_body(struct qm3x_coredump *cd,
				     struct sk_buff *skb)
{
	struct device *dev = cd->dev;
	size_t length = skb->len;
	...
	csum = qm3x_coredump_csum(cdd);
	dev_info(dev, "Coredump received, csum: 0x%x, header csum: 0x%x\n",
		 csum, cdd->csum);
	if (csum == cdd->csum)
		cdd->status |= COREDUMP_RCV_ACK;
	qm3x_coredump_send_rcv_status(cd, cdd->status & COREDUMP_RCV_ACK);
	...
	spin_lock(&cd->dump_lock);
	if (cd->dump_cnt > qm3x_coredumps_max) {
	...
	cdd->bin_attr.read = qm3x_coredumpx_read;
	...
	if (ret) {
		...
		INIT_WORK(&cdd->rfw, qm3x_coredumpx_remove);
...
/**
 * qm3x_coredump_handle_pkt() - Handler one COREDUMP packet.
 * @cd: Pointer to struct qm3x_coredump.
 * @skb: The COREDUMP packet to process.
 *
 * Context: Called from qm3x_transport_event() with received packet.
 */
static void qm3x_coredump_handle_pkt(struct qm3x_coredump *cd,
				     struct sk_buff *skb)
{
	struct device *dev = cd->dev;
	enum qm3x_coredump_cmd_id cmd;
	size_t length = skb->len;

	if (length < sizeof(uint8_t))
		goto error;
	/* Get received command. */
	cmd = (enum qm3x_coredump_cmd_id)skb->data[0];
	skb_pull(skb, sizeof(uint8_t));
	/* Handle command. */
	switch (cmd) {
	case COREDUMP_HEADER_NTF:
		qm3x_coredump_handle_header(cd, skb);
		break;
	case COREDUMP_BODY_NTF:
		qm3x_coredump_handle_body(cd, skb);
		break;
	default:
...
/**
 * qm3x_coredump_event_cb() - Packet handler to handle COREDUMP packets.
 * @data: Pointer to qm3x_coredump structure.
 * @skb: COREDUMP packet received.
 *
 * It is just a simple wrapper to call qm3x_coredump_handle_pkt().
 *
 * Context: Always called from qm3x_transport_event().
 */
static void qm3x_coredump_event_cb(void *data, struct sk_buff *skb)
{
	qm3x_coredump_handle_pkt((struct qm3x_coredump *)data, skb);
}

/**
 * qm3x_coredumpx_remove() - Async remove coredumpX buffer and sysfs file.
 * @work: Pointer to rfw field inside struct qm3x_coredump_data.
 *
...
{
	struct qm3x_coredump_data *cdd =
		container_of(work, struct qm3x_coredump_data, rfw);
	struct qm3x_coredump *cd = cdd->bin_attr.private;
	struct device *dev = cd->dev;
...
/**
 * qm3x_coredumpx_read() - Read coredumpX buffer.
...
{
	struct qm3x_coredump_data *cdd =
		container_of(bin_attr, struct qm3x_coredump_data, bin_attr);
	int ret;
...
/**
 * qm3x_coredump_read() - Read available coredump list.
 * @filp: The struct file instance.
 * @kobp: Device kernel object associated.
 * @bin_attr: Pointer to written binary attribute.
 * @buf: Pointer to application buffer.
 ...
 * Returns: read size on success, else a negative error code.
 */
static ssize_t qm3x_coredump_read(struct file *filp, struct kobject *kobp,
				  struct bin_attribute *bin_attr, char *buf,
				  loff_t pos, size_t count)
{
	struct qm3x_coredump *cd =
		container_of(bin_attr, struct qm3x_coredump, bin_attr);
	struct qm3x_coredump_data *cdd;
	int ret = 0, len = 0;
...
/**
 * qm3x_coredump_write() - Write coredump command.
 * @filp: The struct file instance.
 * @kobp: Device kernel object associated.
 * @bin_attr: Pointer to written binary attribute.
 ...
 */
static ssize_t qm3x_coredump_write(struct file *filp, struct kobject *kobp,
				   struct bin_attribute *bin_attr, char *buf,
				   loff_t pos, size_t count)
{
	struct qm3x_coredump *cd =
...
/**
 * qm3x_coredump_register() - Register a coredump extension.
 * @qm35: The associated QM35 device.
 *
 * Allocate required struct qm3x_coredump and register the COREDUMP transport
 * callback to receive COREDUMP packets as soon as possible.
 *
 * Context: User-space process (modprobe).
 * Returns: 0 on success, else a negative error code.
 */
static int qm3x_coredump_register(struct qm3x *qm35)
{
	struct device *dev = qm3x_get_device(qm35);
	struct qm3x_coredump *coredump;
	int rc;
	...
	INIT_DELAYED_WORK(&coredump->dwork, qm3x_coredump_check);
	ratelimit_state_init(&coredump->dump_rls, COREDUMPS_RATELIMIT_INTERVAL,
			     qm3x_coredumps_burst);
	...
	rc = qm3x_transport_register(qm35, QM3X_TRANSPORT_MSG_COREDUMP,
				     QM3X_TRANSPORT_PRIO_NORMAL,
				     qm3x_coredump_event_cb, coredump);
	if (rc < 0)
		goto error_free;
...
/**
 * qm3x_coredump_online() - Finalize setup for a COREDUMP instance.
 * @qm35: The associated QM35 device.
 *
 ...
 * Returns: 0 on success, else a negative error code.
 */
static int qm3x_coredump_online(struct qm3x *qm35)
{
	struct qm3x_coredump *coredump = qm3x_coredump_search(qm35);
	struct device *dev;
	int rc;
	...
	coredump->bin_attr.read = qm3x_coredump_read;
	coredump->bin_attr.write = qm3x_coredump_write;
...
/**
 * qm3x_coredump_deregister() - Deregister a coredump extension.
 * @qm35: The associated QM35 device.
 *
 * Context: User-space process (modprobe).
 */
static void qm3x_coredump_deregister(struct qm3x *qm35)
{
	struct qm3x_coredump *coredump = qm3x_coredump_search(qm35);
	struct qm3x_coredump_data *cdd;
	struct device *dev;
	...
	qm3x_transport_unregister(qm35, QM3X_TRANSPORT_MSG_COREDUMP,
				  QM3X_TRANSPORT_PRIO_NORMAL,
				  qm3x_coredump_event_cb);
	...
	do {
		...
		cdd = list_first_entry_or_null(&coredump->dump_list,
					       struct qm3x_coredump_data, list);
		if (!cdd) {
		...
		/* Cannot use qm3x_coredump_free_data(cdd) here. We must remove
		 * from list first to allow unlock dump_lock, then remove file.
		 * Cannot call sysfs_remove_bin_file() with dump_lock locked.
		 * This generate a BUG dumpstack with CONFIG_LOCKDEP enabled.
		 * [ BUG: Invalid wait context ]
		 * kunit_try_catch/742 is trying to lock:
		 * 0000000085811940 (&root->kernfs_rwsem){++++}-{3:3}, at:
		 *     kernfs_add_one+0x6d/0x300
		 */
		list_del(&cdd->list);
		coredump->dump_cnt--;
		spin_unlock(&coredump->dump_lock);
...
/**
 * qm3x_coredump_notifier() - The struct notifier_block callback function.
 ...
 */
static int qm3x_coredump_notifier(struct notifier_block *nb,
				  unsigned long action, void *data)
{
	...
	case QM3X_NOTIFIER_EVENT_NEW:
		...
		qm3x_coredump_register(qm35);
		break;
	case QM3X_NOTIFIER_EVENT_ONLINE:
		/* Ignore error, don't want to stop notifier chain. */
		qm3x_coredump_online(qm35);
		break;
	case QM3X_NOTIFIER_EVENT_DELETE:
		qm3x_coredump_deregister(qm35);
		break;
	default:
		rc = -EINVAL;
		break;
...
static struct notifier_block nb = {
	.notifier_call = qm3x_coredump_notifier,
	.next = NULL,
	.priority = 0,
};

/**
 * qm3x_coredump_init() - Module init function.
 *
 * Context: User context.
 * Returns: 0 on success, else qm3x_register_notifier() error code.
 */
static int qm3x_coredump_init(void)
{
	return qm3x_register_notifier(&nb);
}

/**
 * qm3x_coredump_exit() - Module exit function.
 *
 * Context: User context.
 * Returns: 0 on success, else qm3x_unregister_notifier() error code.
 */
static int qm3x_coredump_exit(void)
{
	struct qm3x_coredump *cur, *n;

	list_for_each_entry_safe (cur, n, &coredumps_list, dev_list) {
		qm3x_coredump_deregister(cur->qm35);
	}
	return qm3x_unregister_notifier(&nb);
}

#ifndef QM3X_COREDUMP_TESTS

static int __init qm3x_coredump_module_init(void)
{
	return qm3x_coredump_init();
}

static void __exit qm3x_coredump_module_exit(void)
{
	qm3x_coredump_exit();
}

module_init(qm3x_coredump_module_init);
module_exit(qm3x_coredump_module_exit);
...
MODULE_ALIAS("qm35_coredump");

#endif /* !QM3X_COREDUMP_TESTS */
