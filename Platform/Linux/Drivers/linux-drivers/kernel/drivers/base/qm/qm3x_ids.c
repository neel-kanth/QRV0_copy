...
static DEFINE_IDR(qm3x_ids);
static DEFINE_MUTEX(qm3x_ids_lock);

/**
 * qm3x_new_id() - allocate a new device id for a qm35 struct
...
int qm3x_new_id(struct qm3x *qm35)
{
	...
	if (qm35 == NULL)
		return -EINVAL;
	mutex_lock(&qm3x_ids_lock);
	rc = idr_alloc(&qm3x_ids, qm35, 0, QM35_NUM_DEVICES, GFP_KERNEL);
	mutex_unlock(&qm3x_ids_lock);
	if (rc < 0)
		return rc;
	qm35->dev_id = rc;
	return 0;
...
/**
 * qm3x_remove_id() - delete a previously allocated device id
...
int qm3x_remove_id(int dev_id)
{
	...
	mutex_lock(&qm3x_ids_lock);
	qm35 = idr_remove(&qm3x_ids, dev_id);
	mutex_unlock(&qm3x_ids_lock);
	if (qm35 == NULL)
		return -EINVAL;
	return 0;
...
/**
 * qm3x_find_by_id() - returns a qm35 instance from its id
...
/**
 * qm3x_foreach_id() - iterates over all the q35 instances
...
int qm3x_foreach_id(qm3x_ids_cb cb, void *cb_arg)
{
	...
	mutex_lock(&qm3x_ids_lock);
	idr_for_each_entry (&qm3x_ids, qm35, dev_id) {
		rc = cb(qm35, cb_arg);
		if (rc)
			break;
	}
