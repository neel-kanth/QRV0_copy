...
/**
 * qm3x_transport_start() - Calls the start callback.
...
EXPORT_SYMBOL(qm3x_transport_start);

/**
 * qm3x_transport_stop() - Calls the stop callback.
...
EXPORT_SYMBOL(qm3x_transport_stop);

/**
 * qm3x_transport_reset() - Calls the reset callback.
 * @qm35: QM35 instance to reset.
 * @bootrom: Reset to bootrom command mode.
 *
 * This function is used by the ``qm3x_bypass_control()`` function to request
...
/**
 * qm3x_transport_fw_update() - Calls the firmware update callback.
 ...
 *
 * This function is used by the ``qm3x_register_device()`` function to update the
...
/**
 * qm3x_transport_set_irq() - Calls the set IRQ callback.
 * @qm35: QM35 instance to manage IRQ.
 * @on: Int that define if we enable or disable IRQ.
 *
 * This function is used by the ``qm3x_bypass_control()`` function to enable or
 * disable the IRQ line of the QM35 device.
 */
void qm3x_transport_set_irq(struct qm3x *qm35, int on)
{
	if (qm35->transport_ops->set_irq)
		qm35->transport_ops->set_irq(qm35, on);
}

/**
 * qm3x_transport_wait_irq() - Calls the wait IRQ callback.
 * @qm35: QM35 instance to wait for IRQ.
 * @timeout_ms: Timeout in milliseconds.
 *
 * This function is used by the ``qm3x_bypass_control()`` function to wait for
 * the IRQ line to be asserted.
 *
 * Returns: Zero or a negative error from wait IRQ callback.
 */
int qm3x_transport_wait_irq(struct qm3x *qm35, int timeout_ms)
{
	int rc = 0;

	if (qm35->transport_ops->wait_irq) {
		rc = qm35->transport_ops->wait_irq(qm35, timeout_ms);
	}
	return rc;
}

/**
 * qm3x_transport_spi_transfer() - Calls the SPI transfer callback.
 * @qm35: QM35 instance to do a SPI transfer.
 * @xfer: The SPI transfer structure.
 *
 * This function is used by the ``qm3x_bypass_control()`` function to perform a
 * SPI transfer to the QM35 device.
 *
 * Returns: Zero or a negative error from SPI transfer callback.
 */
int qm3x_transport_spi_transfer(struct qm3x *qm35, struct spi_transfer *xfer)
{
	int rc = 0;

	if (qm35->transport_ops->spi_transfer) {
		rc = qm35->transport_ops->spi_transfer(qm35, xfer);
	}
	return rc;
}

/**
 * qm3x_transport_power() - Calls the power callback.
...
/**
 * qm3x_transport_has_high_prio() - Check high-prio handler registered.
 * @qm35: The QM35 device.
 * @type: The type of message.
 *
 * Return: 1 if an high-prio handler is registered else 0.
 */
static int qm3x_transport_has_high_prio(struct qm3x *qm35,
					enum qm3x_transport_msg_type type)
{
	...
	idx = type * QM3X_TRANSPORT_PRIO_COUNT + QM3X_TRANSPORT_PRIO_HIGH;

	/* To avoid race condition in qm3x_transport_event(), we need to
...
/**
 * qm3x_transport_send() - Send a packet to the qm35 transport layer.
 * @qm35: The qm35 device.
 ...
 * transport drivers in its ``struct qm3x_transport_ops`` structure.
 *
 * This function may be called BEFORE the ``qm3x_transport_start()`` was called,
...
{
	int ret = qm3x_transport_has_high_prio(qm35, type);
	if (ret)
		return ret < 0 ? ret : -EBUSY;
	return qm35->transport_ops->send(qm35, type, data, length);
}
EXPORT_SYMBOL(qm3x_transport_send);

/**
 * qm3x_transport_get_registered() - Get registered callback for a type.
 * @qm35: The QM35 device.
 * @type: The type of message.
 ...
 */
static int
qm3x_transport_get_registered(struct qm3x *qm35,
			      enum qm3x_transport_msg_type type,
			      struct qm3x_transport_recv_handler *out)
{
	int ret = 0;
	unsigned idx;
...
/**
 * qm3x_transport_probe() - Probe the device.
 * @qm35: The QM35 device.
...
/**
 * qm3x_transport_event() - Receive an event from the transport layer.
 * @qm35: The qm35 device.
 ...
 */
int qm3x_transport_event(struct qm3x *qm35,
			 enum qm3x_transport_events transport_event)
{
	struct qm3x_transport_recv_handler handler;
	enum qm3x_transport_msg_type type;
	struct sk_buff *skb;
	int flags, rc;
	...
	if (qm3x_transport_get_registered(qm35, type, &handler) < 0) {
		rc = -EOPNOTSUPP;
		goto error;
	}
...
EXPORT_SYMBOL(qm3x_transport_event);

/**
 * qm3x_transport_register() - Register a callback for a message type.
 * @qm35: The QM35 device.
 ...
 *
 * Registered callbacks are called by ``qm3x_transport_event()`` according to
			    ...
			    qm3x_transport_recv_cb callback, void *data)
{
	...
	if (!qm35 || !callback || type >= QM3X_TRANSPORT_MSG_MAX ||
	    prio >= QM3X_TRANSPORT_PRIO_COUNT)
		return -EINVAL;
	/* No test for idx since type & prio already tested. */
	idx = type * QM3X_TRANSPORT_PRIO_COUNT + prio;
...
EXPORT_SYMBOL(qm3x_transport_register);

/**
 * qm3x_transport_unregister() - Unregister the callback for a message type.
 * @qm35: The QM35 device.
 * @type: The type of msg received.
...
EXPORT_SYMBOL(qm3x_transport_unregister);
