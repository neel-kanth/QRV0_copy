...
int qm35_hsspi_wakeup(struct qm35_spi *qmspi, bool force)
{
	...
	if (qmspi->wakeup_gpio) {
		...
		/* Setup a dummy transaction using 2 transfers! */
		/* This is required because without a correct transfer, the
		 * controller driver's transfer_one() function isn't called, and
		 * if it doesn't provide the set_cs() callback, the CS line isn't
		 * touched at all. */
		memset(xfer.tr, 0, sizeof(xfer.tr));
		spi_message_init_with_transfers(&xfer.msg, xfer.tr, 2);
		xfer.tr[0].tx_buf = &dummydata[0];
		xfer.tr[0].len = sizeof(dummydata[0]);
		xfer.tr[1].tx_buf = &dummydata[1];
		xfer.tr[1].len = sizeof(dummydata[1]);
		/* Add a delay after last transfer. See spi_transfer_delay_exec()
		 * called by spi_transfer_one_message(). */
#if (KERNEL_VERSION(5, 13, 0) > LINUX_VERSION_CODE)
		xfer.tr[1].delay_usecs = delay_gpioless;
#else
		xfer.tr[1].delay.unit = SPI_DELAY_UNIT_USECS;
		xfer.tr[1].delay.value = delay_gpioless;
#endif
 ...
 * Return: 0 on success, else a negative error code.
 */
static int qm35_hsspi_wait_ready(struct qm35_spi *qmspi)
{
