...
const char *const optionnal_modules[] = { "qm3x_logs", "qm3x_coredump",
					  "qm3x_uci_dev" };
 ...
 * event has been received by calling the qm3x_transport_event() function.
 *
...
{
	enum qm3x_transport_events event = QM3X_TRANSPORT_EVENT_IRQ;

	trace_qm35_spi_isr(qmspi);
	qm3x_transport_event(&qmspi->base, event);
 ...
 *
 * This function allocates private device structure with qm3x_alloc_device()
 * and then initializes all HW related stuff (SPI, GPIOs, IRQ...).
 *
 * Then, if there are no errors, the device is registered with
 * qm3x_register_device() which also starts the probing using the provided
 * transport callbacks.
 *
...
static int qm35_spi_driver_probe(struct spi_device *spi)
{
	...
	struct qm3x_transport transport = qm35_spi_transport;
	...
	QM35_CONFIGURE_PARAMETER(debug_flags, u32, "%#x", spi->dev, node);
	...
	if (qm35_debug_flags & QMSPI_NO_PROBING) {
		transport.flags |= QM3X_TRANSPORT_NO_PROBING;
	}
#else
	transport.flags |= QM3X_TRANSPORT_NO_PROBING;
#endif
	qm = qm3x_alloc_device(&spi->dev, sizeof(*qmspi), &transport);
	if (!qm) {
		rc = -ENOMEM;
		goto err_alloc_hw;
	}
	...
	dev_info(qm->dev, "Setup mode: %d, %u bits/w, %u Hz max, can_dma: %d\n",
		 ...
		 spi->max_speed_hz, spi->controller->can_dma != NULL);
