...
static const char *const qm35##series##_##revision##_fw_list[] = {         \
	...
	CONFIG_QM35_FIRMWARE_DIR "qm35_fw_pkg_prod.bin",                   \
	CONFIG_QM35_FIRMWARE_DIR "qm35_fw_pkg.bin",                        \
...
#if IS_ENABLED(CONFIG_QM35_PROBING)
			 ...
			 size_t count)
{
	...
	ret = memory_read_from_buffer(buf, count, &pos, &qmspi->info_buf,
				      qmspi->info_length);
 ...
 */
static int qm35_spi_start(struct qm3x *qm35)
{
 ...
 */
static void qm35_spi_stop(struct qm3x *qm35)
{
	struct qm35_spi *qmspi = qm35_to_qm35_spi(qm35);
	struct device *dev = qmspi->base.dev;
	int rc;
...
static int qm35_spi_set_cs_level(struct qm35_spi *qmspi, bool level)
{
	...
	trace_qm35_spi_set_cs_level(qmspi, level);
...
/**
 * qm35_reset_work() - Worker function for resetting the device.
 * @qmspi: QM35 SPI instance to issue reset.
 * @in: Worker function input parameters.
 * @out: Worker function output buffer.
 *
 * This function is used to ensure GPIO and CS level toggling is done from
 * inside the qm35_spi_thread() function, to avoid concurrency with send and
 * receive operations.
 *
 * Returns: Zero on success else a negative error code.
 */
static int qm35_reset_work(struct qm35_spi *qmspi, const void *in, void *out)
{
	struct device *dev = qmspi->base.dev;
	bool bootrom = *(bool *)in;
	...
	usleep_range(QM35_RESET_DURATION_US, QM35_RESET_DURATION_US + 100);
	...
	if (!bootrom) {
		...
		usleep_range(QM35_BOOTROM_RESET_BACKOFF_DURATION_US,
	...
	/* Reset qm3x_state. */
	qm3x_state_set(&qmspi->base, QM3X_STATE_UNKNOWN);
	...
	while (loop) {
		...
		if (header.flags & HSSPI_SOC_RDY) {
			...
			if (header.flags & HSSPI_SOC_ODW &&
			    header.ul_value == QM3X_TRANSPORT_MSG_COREDUMP)
...
int qm35_spi_reset_wait_ready(struct qm35_spi *qmspi, bool bootrom, bool wait)
{
	...
	trace_qm35_spi_reset(qmspi, bootrom);
	disable_irq(qmspi->spi->irq);

	/* Enqueue reset work. */
	memset(&work, 0, sizeof(work));
	work.cmd = qm35_reset_work;
	work.in = &bootrom;
	qm35_enqueue(qmspi, &work);
 ...
 * Returns: Zero on success else a negative error code.
 */
static int qm35_spi_reset(struct qm3x *qm35, bool bootrom)
{
	struct qm35_spi *qmspi = qm35_to_qm35_spi(qm35);
 ...
 */
static int qm35_spi_fw_update_single(struct qm3x *qm35,
				     const struct qm3x_fw_version *current_ver,
				     const char *fw_name, bool force)
{
	struct qm35_spi *qmspi = qm35_to_qm35_spi(qm35);
	struct qm3x_fw_version fw_ver;
	...
	if (!qm35_fw_get_vendor_version(qmspi, &fw_ver)) {
		qm3x_fw_version_print(dev_info, qm35->dev,
	...
	if (force) {
		...
		if (qm3x_fw_version_cmp(current_ver, &fw_ver)) {
			run_fw_upgrade = true;
		} else {
			dev_info(
				qm35->dev,
 ...
 * * else a negative error code.
 */
static int qm35_spi_fw_update(struct qm3x *qm35,
			      struct qm3x_fw_version *current_ver,
...
/**
 * qm35_spi_set_irq() - QM35 transport set IRQ callback.
 * @qm35: QM35 core instance to manage IRQ.
 * @on: Int that decides if we enable or disable IRQ.
 *
 * Enable or disable IRQ for the QM35 SPI device.
 *
 * Used to disable IRQ while flashing using the QM35 firmware updater, as it
 * does not comply with the standard HSSPI STC protocol and keeps the IRQ line
 * high.
 */
static void qm35_spi_set_irq(struct qm3x *qm35, int on)
{
	struct qm35_spi *qmspi = qm35_to_qm35_spi(qm35);

	trace_qm35_spi_set_irq(qmspi, on);
...
/**
 * qm35_spi_wait_irq() - QM35 transport wait IRQ callback.
 * @qm35: QM35 core instance to wait IRQ.
 * @timeout_ms: Timeout in milliseconds.
 *
 * Wait for the IRQ line of the QM35 SPI device to be asserted.
 *
 * Used to wait for the QM35 to be ready when IRQ is disabled, for example
 * during the firmware updater process.
 *
 * Returns: Zero on success else a negative error code.
 */
static int qm35_spi_wait_irq(struct qm3x *qm35, int timeout_ms)
{
	struct qm35_spi *qmspi = qm35_to_qm35_spi(qm35);
	struct gpio_desc *irq_gpio = qmspi->irq_gpio;
	bool irq;
	int ret;
	...
	trace_qm35_spi_wait_irq(qmspi, timeout_ms);
	...
	ret = irq ? 0 : -ETIMEDOUT;
	trace_qm35_spi_wait_irq_return(qmspi, ret);
	return ret;
}

static int qm35_spi_transfer_work(struct qm35_spi *qmspi, const void *in,
				  void *out)
{
	struct spi_transfer *xfer = (struct spi_transfer *)in;

	return spi_sync_transfer(qmspi->spi, xfer, 1);
}

/**
 * qm35_spi_transfer() - QM35 transport transfer callback.
 * @qm35: QM35 core instance to transfer data.
 * @xfer: SPI transfer structure to transfer.
 *
 * Transfer data to the QM35 SPI device.
 *
 * Calls spi_sync_transfer() through the high-prio thread to transfer data to
 * the QM35 HW.
 *
 * Used to transfer data to the QM35 during the firmware updater process, which
 * does not comply with the standard HSSPI STC protocol.
 *
 * Returns: Zero on success else a negative error code.
 */
static int qm35_spi_transfer(struct qm3x *qm35, struct spi_transfer *xfer)
{
	struct qm35_spi *qmspi = qm35_to_qm35_spi(qm35);
	struct qm35_work work = { .cmd = qm35_spi_transfer_work, .in = xfer };
	int ret = 0;

	trace_qm35_spi_transfer(qmspi);

	/* Enqueue SPI transfer work. */
	ret = qm35_enqueue(qmspi, &work);
	trace_qm35_spi_transfer_return(qmspi, ret);
	return ret;
}
 ...
 * Returns: Zero on success else a negative error code.
 */
static int qm35_spi_power(struct qm3x *qm35, int on)
{
	struct qm35_spi *qmspi = qm35_to_qm35_spi(qm35);
	int rc = 0;
 ...
 * Returns: Zero on success else a negative error code.
 */
static int qm35_spi_send(struct qm3x *qm35, enum qm3x_transport_msg_type type,
			 const void *data, size_t size)
{
	...
	irq_count = atomic_read(&qmspi->irq_count);
	awake_count = atomic_read(&qmspi->awake_count);
	ret = qm35_enqueue(qmspi, &qmspi->work_send);
	while ((ret == -EAGAIN || ret == -EBUSY || ret == -EINPROGRESS) &&
	       retry_count) {
		if (ret == -EINPROGRESS) {
			 ...
			 * packet.
			 * If QM has woken-up BEFORE the following wait event
			 * is called, using a counter allows to not wait.
			 */
			int rc;
			rc = wait_event_interruptible_hrtimeout(
				qmspi->wakeup_wait,
				awake_count != atomic_read(&qmspi->awake_count),
				ns_to_ktime(QM35_WAKEUP_DELAY_US * 2000));
			trace_qm35_spi_send_awake(qmspi, rc);
			/* If async wakeup used, don't touch retry_count. */
			if (rc == -ETIME && !qmspi->exton_gpio) {
				/* Forced wakeup seems to have failed. Retry it. */
				ret = -EBUSY;
				wakeup_forced = false;
				/* Count forced async wakeup as a try to avoid endless loop! */
				retry_count--;
			}
		} else {
			...
			retry_udelay *= 2;
			/* A try was made but failed. */
			retry_count--;
		}
		if (!qmspi->exton_gpio) {
			int new_irq_count = atomic_read(&qmspi->irq_count);
			bool allows_force = new_irq_count == irq_count;
			qmspi->send_params.wakeup = false;
			/* Update send params to force wakeup if needed. */
			if (ret == -EBUSY && allows_force && !wakeup_forced) {
...
static int qm35_recv_work(struct qm35_spi *qmspi, const void *in, void *out)
{
	...
	if (ret < 0)
		return ret;
	params->type = (enum qm3x_transport_msg_type)header.ul_value;
 ...
 *
 * This function is always called from the qm3x_transport_event() in reply
 ...
 */
static ssize_t qm35_spi_recv(struct qm3x *qm35, void *data, size_t size,
 ...
 * for the device boot notification.
 *
 * It retry a hard-reset command using qm35_spi_reset() if there is no boot
 * notification after the initial hard-reset.
 *
 ...
 */
static int qm35_spi_probe(struct qm3x *qm35, char *infobuf, size_t len)
{
#if IS_ENABLED(CONFIG_QM35_PROBING)
	struct qm35_spi *qmspi = qm35_to_qm35_spi(qm35);
	...
	while (qm3x_state_wait(qm35, QM3X_STATE_BOOTED) <= 0) {
		/* Exit loop with error if second try failed. */
		if (reset_retried) {
			rc = -ETIMEDOUT;
			goto cleanup_probe;
		}
		/* Issue another HARD reset if there is no boot notification
		 * after the initial HARD reset made before this function is
		 * called. */
		rc = qm35_spi_reset(qm35, false);
		if (rc)
			/* Force the firmware upgrade if device fail to become
			 * ready after the HARD reset. */
			goto cleanup_probe;
		reset_retried = true;
	}

	/* After device boot notification is received, QM35 device can answer
	 * a CORE_GET_DEVICE_INFO command. */
	rc = qm35_uci_probe_device_info(
		qmspi, (struct qm3x_uci_device_info *)infobuf, len);
	if (rc)
		goto cleanup_probe;

	mutex_lock(&qmspi->info_mutex);
	/* Save info */
	qmspi->info_length = min(
		sizeof(qmspi->info_buf),
		sizeof(struct qm3x_uci_device_info) +
			((struct qm3x_uci_device_info *)infobuf)->vendor_length);
	memcpy(&qmspi->info_buf, infobuf, qmspi->info_length);
	...
	qmspi->info_bin_attr.size = qmspi->info_length;
...
cleanup_probe:
	...
	return rc;
#else
	dev_warn(qm35->dev, "Device probing disabled at built time.\n");
	return 0;
#endif
}

static const struct qm3x_transport_ops qm35_spi_transport_ops = {
	...
	.set_irq = qm35_spi_set_irq,
	.wait_irq = qm35_spi_wait_irq,
	.spi_transfer = qm35_spi_transfer,
	.power = qm35_spi_power,
	.probe = qm35_spi_probe,
};

const struct qm3x_transport qm35_spi_transport = {
	.name = "qm35_spi",
