...
/* Maximum data length to display in traces is 263:
 * - 4 bytes of HSSPI STC header;
 * - 4 bytes of UCI header;
 * - 255 bytes of payload.
 */
#define DATA_MAX_LEN 263
#define DATA_ENTRY __dynamic_array(u8, data, len)
#define DATA_ASSIGN \
	memcpy(__get_dynamic_array(data), data, __get_dynamic_array_len(data))
#define DATA_PR_FMT ", data: %s%s"
#define DATA_PR_ARG                                                \
	__print_hex(__get_dynamic_array(data),                     \
		    __get_dynamic_array_len(data) < DATA_MAX_LEN ? \
...
DECLARE_EVENT_CLASS(qms_evt_with_arg,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg),
	TP_STRUCT__entry(
		QMS_ENTRY
		__field(int, arg)
	...
	TP_printk(QMS_PR_FMT ", return: %d", QMS_PR_ARG, __entry->arg)
);
...
DEFINE_EVENT(qms_evt_with_arg, qm35_spi_start_return,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg)
);
...
DEFINE_EVENT(qms_evt_with_arg, qm35_spi_stop_return,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg)
);

DEFINE_EVENT_PRINT(qms_evt_with_arg, qm35_spi_set_cs_level,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg),
	TP_printk(QMS_PR_FMT ", level: %d", QMS_PR_ARG, __entry->arg)
);

DEFINE_EVENT(qms_evt_with_arg, qm35_spi_set_cs_level_return,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg)
);

DEFINE_EVENT_PRINT(qms_evt_with_arg, qm35_spi_reset,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg),
	TP_printk(QMS_PR_FMT ", bootloader: %d", QMS_PR_ARG, __entry->arg)
);

DEFINE_EVENT(qms_evt_with_arg, qm35_spi_reset_return,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg)
);
...
DEFINE_EVENT(qms_evt_with_arg, qm35_spi_fw_update_single_return,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg)
);
...
DEFINE_EVENT(qms_evt_with_arg, qm35_spi_fw_update_return,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg)
);

DEFINE_EVENT_PRINT(qms_evt_with_arg, qm35_spi_set_irq,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg),
	TP_printk(QMS_PR_FMT ", IRQ: %s", QMS_PR_ARG, __entry->arg ? "ON" : "OFF")
);

DEFINE_EVENT_PRINT(qms_evt_with_arg, qm35_spi_wait_irq,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg),
	TP_printk(QMS_PR_FMT ", timeout: %d", QMS_PR_ARG, __entry->arg)
);

DEFINE_EVENT(qms_evt_with_arg, qm35_spi_wait_irq_return,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg)
);

DEFINE_EVENT(qms_only_evt, qm35_spi_transfer,
	TP_PROTO(struct qm35_spi *qms),
	TP_ARGS(qms)
);

DEFINE_EVENT(qms_evt_with_arg, qm35_spi_transfer_return,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg)
);

DEFINE_EVENT_PRINT(qms_evt_with_arg, qm35_spi_power,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg),
	TP_printk(QMS_PR_FMT ", power: %s", QMS_PR_ARG, __entry->arg ? "ON" : "OFF")
);

DEFINE_EVENT(qms_evt_with_arg, qm35_spi_power_return,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg)
);
...
DEFINE_EVENT(qms_evt_with_arg, qm35_spi_send_return,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg)
);

DEFINE_EVENT_PRINT(qms_evt_with_arg, qm35_spi_send_awake,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg),
	TP_printk(QMS_PR_FMT ", ret: %d", QMS_PR_ARG, __entry->arg)
);
...
DEFINE_EVENT(qms_evt_with_arg, qm35_spi_recv_return,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg)
);
...
DEFINE_EVENT(qms_evt_with_arg, qm35_spi_probe_return,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg)
);
...
DEFINE_EVENT(qms_evt_with_arg, qm35_fw_upgrade_work_return,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg)
);
...
DEFINE_EVENT(qms_evt_with_arg, qm35_fw_upgrade_return,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg)
);
...
DEFINE_EVENT(qms_evt_with_arg, qm35_fw_get_device_id_work_return,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg)
);
...
DEFINE_EVENT(qms_evt_with_arg, qm35_fw_get_device_id_return,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg)
);
...
DEFINE_EVENT(qms_evt_with_arg, qm35_fw_load_return,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg)
);
...
DEFINE_EVENT(qms_evt_with_arg, qm35_fw_free_return,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg)
);
...
DEFINE_EVENT(qms_evt_with_arg, qm35_fw_get_vendor_version_return,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg)
);
...
DEFINE_EVENT(qms_evt_with_arg, qm35_hsspi_send_return,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg)
);

DEFINE_EVENT_PRINT(qms_evt_with_arg, qm35_hsspi_recv,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg),
	TP_printk(QMS_PR_FMT ", buflen: %d", QMS_PR_ARG, __entry->arg)
);

DEFINE_EVENT(qms_evt_with_arg, qm35_hsspi_recv_return,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg)
);

DEFINE_EVENT_PRINT(qms_evt_with_arg, qm35_hsspi_wait_ready,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg),
	TP_printk(QMS_PR_FMT ", count: %d", QMS_PR_ARG, __entry->arg)
);
...
DECLARE_EVENT_CLASS(qms_cmd_queue,
	...
	TP_printk(QMS_PR_FMT ", cmd: %ps, in: %p, out: %p",
...
DEFINE_EVENT(qms_evt_with_arg, qm35_enqueue_return,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg)
);
...
DEFINE_EVENT(qms_evt_with_arg, qm35_process_cmd_return,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg)
);
...
DEFINE_EVENT(qms_evt_with_arg, qm35_pm_runtime_suspend_return,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg)
);
...
DEFINE_EVENT(qms_evt_with_arg, qm35_pm_runtime_resume_return,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg)
);
...
DEFINE_EVENT(qms_evt_with_arg, qm35_pm_runtime_idle_return,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg)
);
...
DEFINE_EVENT(qms_evt_with_arg, qm35_system_suspend_return,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg)
);
...
DEFINE_EVENT(qms_evt_with_arg, qm35_system_resume_return,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg)
);
...
DEFINE_EVENT(qms_evt_with_arg, qmrom_spi_transfer_return,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg)
);

DEFINE_EVENT_PRINT(qms_evt_with_arg, qmrom_spi_set_cs_level,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg),
	TP_printk(QMS_PR_FMT ", level: %d", QMS_PR_ARG, __entry->arg)
);

DEFINE_EVENT(qms_evt_with_arg, qmrom_spi_set_cs_level_return,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg)
);
...
DEFINE_EVENT(qms_evt_with_arg, qmrom_spi_wait_for_irq_line_return,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg)
);
...
DEFINE_EVENT(qms_evt_with_arg, qmrom_spi_read_irq_line_return,
	TP_PROTO(struct qm35_spi *qms, int arg),
	TP_ARGS(qms, arg)
);
