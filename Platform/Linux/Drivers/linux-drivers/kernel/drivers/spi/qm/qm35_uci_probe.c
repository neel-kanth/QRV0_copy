...
static inline int uci_status_to_err(enum uci_status_code status)
{
	...
	return -(int)status - ((int)status ? 10000 : 0);
}
 ...
 *
 * Context: Called from inside the qm3x_transport_event() which receive the
					 ...
					 uint16_t payload_len)
{
	...
	enum qm3x_state new_state = QM3X_STATE_UNKNOWN;
	...
	case UCI_DEVICE_STATE_READY:
		new_state = QM3X_STATE_READY;
		break;
	case UCI_DEVICE_STATE_ACTIVE:
		new_state = QM3X_STATE_ACTIVE;
		break;
	case UCI_DEVICE_STATE_ERROR:
		new_state = QM3X_STATE_ERROR;
		break;
	}
	qm3x_state_set(&qmspi->base, new_state);
	return 0;
}
 ...
 * device information is saved in the struct qm3x_uci_device_info given by the
 * user_data field.
 *
...
static int uci_device_info_status(struct qm35_uci_probing *qmup,
				  ...
				  uint16_t payload_len)
{
	struct qm3x_uci_device_info *info = qmup->cmd_data;
	/* Minimum length is vendor_length = 0, no data, +1 for status. */
	size_t minlen = offsetof(struct qm3x_uci_device_info, vendor_data) + 1;
...
/**
 * uci_device_boot_notification() - Handle device boot notification message.
 * @qmup: The QM35 UCI probing instance.
 * @skb: The UCI frame received.
 * @mt_gid_oid: The message id from UCI packet header.
 * @payload_len: The payload length from UCI packet header.
 ...
 * Return: Always 0.
 */
static int uci_device_boot_notification(struct qm35_uci_probing *qmup,
					struct sk_buff *skb,
					uint16_t mt_gid_oid,
					uint16_t payload_len)
{
	...
	/* Receiving this notification is enough to change current state. */
	qm3x_state_set(&qmspi->base, QM3X_STATE_BOOTED);

	/* Retrieve and show boot reason */
	if (skb->len) {
		uint8_t boot_reason = skb->data[0];
		dev_info(qmspi->base.dev,
			 "Probe: Device booted (reason = %#x).\n",
...
static void qm35_uci_probe_handle(void *data, struct sk_buff *skb)
{
	...
	while (skb->len >= UCI_PACKET_HEADER_SIZE) {
		...
		case UCI_MT_GID_OID(UCI_MESSAGE_TYPE_NOTIFICATION, UCI_GID_CORE,
			...
			break;
		case UCI_MT_GID_OID(UCI_MESSAGE_TYPE_NOTIFICATION,
				    UCI_GID_QORVO_EXT2,
				    UCI_OID_QORVO_EXT2_CORE_DEVICE_BOOT):
			rc = uci_device_boot_notification(qmup, skb, mt_gid_oid,
							  payload_len);
			break;
		default:
			dev_info(qmspi->base.dev,
...
void qm35_uci_probe_setup(struct qm35_spi *qmspi)
{
	...
	qm3x_transport_register(&qmspi->base, QM3X_TRANSPORT_MSG_UCI,
				QM3X_TRANSPORT_PRIO_HIGH, qm35_uci_probe_handle,
				qmup);
}
...
{
	qm3x_transport_unregister(&qmspi->base, QM3X_TRANSPORT_MSG_UCI,
 ...
 * struct qm35_uci_client which is inside struct qm3x instance so the response
 ...
 *          same context than the qm3x_transport_event() function which will do
				   ...
				   size_t udata_sz)
{
	...
	/* Send message */
	ret = qm3x_transport_send_direct(&qmspi->base, QM3X_TRANSPORT_MSG_UCI,
					 cmd, cmd_sz);
	/* Wait for response */
