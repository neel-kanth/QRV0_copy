...
int qmrom_spi_transfer(void *handle, char *rbuf, const char *wbuf, size_t size)
{
	...
	if (((struct qm35_hsspi_header *)wbuf)->flags & HSSPI_HOST_RD)
...
int qmrom_spi_set_cs_level(void *handle, int level)
{
	...
	/* The dummydata is required because without a correct transfer len,
	 * the controller driver's transfer_one() function isn't called, and
	 * if it doesn't provide the set_cs() callback, the CS line isn't
	 * touched at all. */
	struct spi_transfer xfer = {
		.tx_buf = &dummydata,
		.rx_buf = NULL,
		.len = sizeof(dummydata),
		.cs_change = !level,
		.speed_hz = qmrom_spi_speed_hz,
	};
