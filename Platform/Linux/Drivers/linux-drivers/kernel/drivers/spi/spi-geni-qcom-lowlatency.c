 ...
 * randomly between 150µs and more than 2.5ms on a DrangonBoard 845c.
 ...
 *
 * With the "Low latency mode", the same transfer can takes 25µs in an almost
 * constant manner.
 *
 * To reduce the latency, the following changes were made:
 *
 ...
 * - Interrupts of the SPI device are completely disabled.
 * - Transfers are made in polling mode, active wait, in the function
 *   spi_geni_transfer_one().
 *
 * The device tree should be changed to include the CS GPIO in the SPI
 * controller node. For example, add this in the &spi2 node:
 *
...
static void get_tx_fifo_watermark_delay_us(struct spi_geni_master *mas)
{
	...
	mas->tx_fifo_watermark_delay_us =
		mas->max_fifo_bytes * 1000000 / mas->actual_hz ?: 1;
	dev_dbg(mas->dev,
		"TX FIFO watermark delay: %u us for %u Hz transfer.\n",
		mas->tx_fifo_watermark_delay_us, mas->actual_hz);
}
			   ...
			   unsigned int *clk_idx, unsigned int *clk_div)
{
	...
	*clk_div = DIV_ROUND_UP(sclk_freq, mas->oversampling * speed_hz);
	 ...
	 */
	if (likely(mas->low_latency)) {
		mas->actual_hz = actual_hz;
	}
...
static void handle_fifo_timeout(struct spi_master *spi, struct spi_message *msg)
{
	...
	/* Canceling require IRQ to be enabled. */
	if (mas->low_latency)
		enable_irq(mas->irq);
...
static bool spi_geni_is_abort_still_pending(struct spi_geni_master *mas)
{
	...
	if (m_irq & m_irq_en) {
		dev_err(mas->dev, "Interrupts pending after abort: %#010x\n",
			m_irq & m_irq_en);
		if (mas->low_latency) {
			/* Temporary enable IRQ so status is cleaned. */
			enable_irq(mas->irq);
			msleep(1);
			disable_irq(mas->irq);
		}
			       ...
			       unsigned int bits_per_word)
{
	...
	if (mas->last_mode == mode && mas->cur_bits_per_word == bits_per_word)
		return; /* No change. */
	...
	mas->cur_bits_per_word = bits_per_word;
}
				     ...
				     unsigned long clk_hz)
{
	...
	if (likely(mas->low_latency)) {
		/* Compute the mandatory tx_fifo_watermark_delay_us. */
...
static int setup_fifo_params(struct spi_device *spi_slv, struct spi_master *spi)
{
	...
	if (mas->last_mode != spi_slv->mode) {
		...
		if (!spi_slv->cs_gpiod) {
			/* Don't touch demux sel config if GPIO used. */
			if (spi_slv->mode & SPI_CS_HIGH)
				demux_output_inv = BIT(spi_slv->chip_select);
			writel(demux_output_inv,
...
static void spi_gsi_rx_callback_result(void *cb,
				       const struct dmaengine_result *result)
{
	spi_gsi_callback_result(cb, result, false);
}

static void spi_gsi_tx_callback_result(void *cb,
				       const struct dmaengine_result *result)
{
			  ...
			  struct spi_device *spi_slv, struct spi_master *spi)
{
	...
	ret = get_spi_clk_cfg(mas->cur_speed_hz, mas, &peripheral.clk_src,
			      &peripheral.clk_div);
	if (ret) {
	...
	if (peripheral.cmd & SPI_RX) {
		...
		gsi->rx_desc = dmaengine_prep_slave_single(
			mas->rx, xfer->rx_dma, xfer->len, DMA_DEV_TO_MEM,
			flags);
	...
	gsi->tx_desc = dmaengine_prep_slave_single(
		mas->tx, xfer->tx_dma, xfer->len, DMA_MEM_TO_DEV, flags);
			       ...
			       struct spi_message *msg)
{
	...
	list_for_each_entry (xfer, &msg->transfers, transfer_list) {
		if (xfer->rx_buf)
			dma_unmap_single(gsi_dev, xfer->rx_dma, xfer->len,
					 DMA_FROM_DEVICE);
		if (xfer->tx_buf)
			dma_unmap_single(gsi_dev, xfer->tx_dma, xfer->len,
					 DMA_TO_DEVICE);
	};
}
...
/* Forward declaration for low-latency mode. */
				 ...
				 struct spi_transfer *xfer)
{
	...
	if (mas->cur_xfer_mode == GENI_SE_FIFO) {
		...
		if (likely(mas->low_latency)) {
			/* Call geni_spi_isr() in a loop to poll IRQ status. */
			do {
				if (mas->tx_rem_bytes)
					udelay(mas->tx_fifo_watermark_delay_us);
				if (!mas->cur_xfer)
					break; /* If cancelled while waiting. */
				geni_spi_isr(mas->irq, spi);
			} while (mas->cur_xfer);
			ret = 0; /* OK, transfer is done. */
		} else {
			ret = 1; /* OK, transfer is in progress. */
		}
	} else {
...
static int spi_geni_probe(struct platform_device *pdev)
{
	...
	mas->se.base = base;
	mas->se.clk = clk;
	/* Ensure setup_fifo_params() do something on first call. */
	mas->last_mode = 0x10000;
	...
	if (likely(low_latency)) {
		/* Ensure we are always in FIFO mode. */
		if (mas->cur_xfer_mode != GENI_SE_FIFO) {
			mas->cur_xfer_mode = GENI_SE_FIFO;
			geni_se_select_mode(&mas->se, GENI_SE_FIFO);
		}
	}
	if (mas->cur_xfer_mode == GENI_SE_FIFO)
	...
	if (likely(low_latency)) {
		...
		mas->max_fifo_bytes = ((mas->tx_fifo_depth - mas->tx_wm) *
				       mas->fifo_width_bits) /
				      BITS_PER_BYTE;
		dev_info(mas->dev, "FIFO size: %d bytes\n",
			 mas->max_fifo_bytes);

		/* Check is the CS GPIO was setup in DT. */
		nb = gpiod_count(dev, "cs");
		if (nb > 0) {
			/* When CS GPIO are defined in the DT for a controller,
			 * it automatically take precedence over the set_cs()
			 * callback. So assume low-latency is wanted.
			 */
			dev_info(mas->dev,
				 "CS GPIO defined for SPI controller.\n");
			mas->low_latency = true;
		} else {
			/* The SPI slave device won't be able to use CS GPIO.
			 * So assume low-latency isn't wanted for this master.
			 */
			mas->low_latency = false;
		}
		dev_warn(mas->dev, "Low latency mode %s.\n",
			 mas->low_latency ? "enabled" : "disabled");
	}

	irqf = irq_get_trigger_type(mas->irq);
	if (mas->low_latency) {
		/* Hook interruption but without auto-enable at lower-level. */
		irqf |= IRQF_NO_AUTOEN;
	}
	/* Request IRQ but don't activate if in low latency mode. Low latency
	 * mode will call geni_spi_isr() directly in a loop to poll. */
	dev_info(mas->dev, "Requesting IRQ %d.\n", mas->irq);
	ret = request_irq(mas->irq, geni_spi_isr, irqf, dev_name(dev), spi);
	if (ret) {
		dev_err(mas->dev, "Failure to request IRQ %d.\n", ret);
		goto spi_geni_probe_runtime_disable;
	}

	/* Should be kept at the end of the function. */
	ret = spi_register_master(spi);
	if (ret) {
		dev_err(mas->dev, "Failure to register SPI controller %d.\n",
			ret);
