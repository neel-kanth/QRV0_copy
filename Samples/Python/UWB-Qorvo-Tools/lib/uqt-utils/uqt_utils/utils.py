...
def encode_z_element(
    z_element_f: int, z_element_m: int, z_element_h: int, z_element_u: int
):
    """Encode Z-element into location vector format"""
    z_element_f &= (1 << 14) - 1
    z_element_m &= (1 << 2) - 1
    z_element_h &= (1 << 24) - 1
    z_element_u &= (1 << 8) - 1
    ...
    return int.from_bytes(z_element.to_bytes(6, "big"), "little")
...
def compute_dl_tdoa_anchor_location_value(
    ...
    """Encode the base part of location into Fira Location vector format"""
    ...
    if presence == 0:  # DT-Anchor location not present
        ...
        App.defs = [
            param for param in App.defs if param[0] != App.DlTdoaAnchorLocationV2
        ]
        location_size = 0
        if coord_type in (0, 2):  # WGS-84 coordinates systems
            if (
                coord_x > 90.0
                or coord_x < -90.0
                or coord_y > 180.0
            ...
            location_size = 13
            location = int.from_bytes(by_all, "big")

        elif coord_type in (1, 3, 4, 5):  # Relative based coordinates systems
            if (
                coord_x > (2**27) - 1
                or coord_x < -(2**27)
                or coord_y > (2**27) - 1
            ...
            raise ValueError(
                f"coord_type=={coord_type}, but should be one of (0,1,2,3,4,5)"
            )
        App.defs.append((App.DlTdoaAnchorLocation, location_size))
        if coord_type in (2, 3, 5):  # types with Z-element
            z_element = encode_z_element(
                z_element_f, z_element_m, z_element_h, z_element_u
            )
            """add Z-element to the base coordinates vector"""
            location_size += 6
            location = z_element << ((location_size - 6) * 8) | location
        App.defs.append((App.DlTdoaAnchorLocationV2, location_size))
        return location
...
def str2bytes(v: str, endianness: str = "big") -> bytes:
