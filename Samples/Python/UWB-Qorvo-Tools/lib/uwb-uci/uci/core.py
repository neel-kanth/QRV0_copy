...
class MT(DynIntEnum):
    ...
    Response = 2
    Notif = 3
    SeTestingCommand = 0b100
    SeTestingResponse = 0b101
    Unknown = 255
...
def get_length(defs, key, value=None):
    for k, length in defs:
        if k == key:
            if callable(length):
                return length(value)
            return length
    ...
    for t, v in tvs:
        ...
        if isinstance(v, list):
            ...
            if isinstance(lengths, list):
                if hasattr(v, "__len__") and len(v) == 32:
                    length = lengths[1]
                else:
                    length = lengths[0]
            else:
...
def tlvs_from_bytes(enum_class, payload):
    ...
    for i in range(n):
        ...
        try:
            l_elem = get_length(enum_class.defs, t, v)
            if isinstance(l_elem, list):
                if l not in l_elem:
                    raise ValueError(
                        f"Invalid size {l} for type {t}, expected: {l_elem}"
                    )
                l_elem = l
        except (AttributeError, ValueError) as e:
            raise Exception(f"Error retrieving expected size for {t}: {e}")
...
class Client:
    ...
    def get_notif_handlers(self):
        return self.notif_handlers

    def set_notif_handlers(self, handlers):
    ...
    def send_control_msg_for_testing(self, mt: int, pbf: int, payload: bytes):
        """This Control Message format shall be used in case of MT values 0b100 or 0b101

        Args:
            mt: Message Type
            pbf: Packet Boundary Flag
            payload: the payload of the msg that will be sent
        """
        header = bytearray([mt << 5 | pbf << 4, 0x00]) + len(payload).to_bytes(
    ...
    def data_received(self, data):
        ...
        while len(data) > 0:
            ...
            if mt == MT.DataPacket or MT.SeTestingResponse:
            ...
            if mt == MT.SeTestingResponse:
                self.se_bridge_packet_received(packet)
            else:
