...
from .fira import Gid, Status, Client, DeviceInfo, uci_codecs, MT, default_codec
...
class DeviceInfo_custom:
    ...
    def decode_qorvo(self):
        ...
        try:
            ...
            if b.remaining_size() != 0:
                self.flavor = b.pop(24).decode().rstrip()
            if b.remaining_size() != 0:
                self.product_id = b.pop_uint(4)
            if b.remaining_size() != 0:
                self.soi_variant = b.pop_uint(4)
            if b.remaining_size() != 0:
                self.rom_revision = b.pop_uint(2)
    ...
    def __str__(self) -> str:
        ...
        if self.vendor_size == 52:
            qmf_version_str = (
                f"{self.qmf_version_major}.{self.qmf_version_minor}.{self.qmf_version_patch}"
                f"-rc.{self.qmf_version_rc}"
            )
        else:
            qmf_version_str = f"{self.qmf_version_major}.{self.qmf_version_minor}-rc{self.qmf_version_rc}"

        rtv += f"""
        QMF version:         {qmf_version_str}
        OEM version:         {self.oem_version_major}.{self.oem_version_minor}.{self.oem_version_revision}"""
        if self.build_job != 0:
            rtv += f"\n        build job:           {self.build_job}"
        rtv += f"""
        soc id:              {self.soc_id}
        ...
        rtv += f"\n        flavor:              {self.flavor}"
        if isinstance(self.product_id, int):
            rtv += f"\n        Product ID:          {self.product_id:08x}"
        if isinstance(self.soi_variant, int):
            rtv += f"\n        SOI variant:         {self.soi_variant}"
        if isinstance(self.rom_revision, int):
            rtv += f"\n        ROM revision:        {self.rom_revision:04X}"
