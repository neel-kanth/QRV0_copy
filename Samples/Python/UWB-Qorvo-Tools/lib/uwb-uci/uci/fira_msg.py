...
from .fira_cap import caps, UnsupportedCap
from .fira_app import TestParam, App
from .utils import Buffer, ExtendableClass
...
class DeviceInfo_fira:
    ...
    def __str__(self) -> str:
        ...
        uci test version:    {self.uci_test_version_major}.{self.uci_test_version_minor}.{self.uci_test_version_maintenance}"""
...
class Caps:
    ...
    def decode_fira(self):
        ...
        buf = self.buffer
        try:
            self.status = Status(buf.pop_uint(1))
            n = buf.pop_uint(1)
            self.caps_list = []
            for i in range(n):
                key = int(buf.pop(1).hex(), 16)
                length = buf.pop_uint(1)
                val = buf.pop(length)
                if key in fira_enums.CapsParameters.as_value_list():
                    cap_enum = fira_enums.CapsParameters(key)
                    cap_obj = caps.get(cap_enum, None)
                    if cap_obj:
                        self.caps_list.append(cap_obj(val))
                else:
                    self.caps_list.append(UnsupportedCap(key, val))
...
class TestConfigSetReq:
    ...
    def from_bytes(self, value: bytes, byteorder="little"):
        ...
        try:
            buf = Buffer(value)
            self.session_handle = buf.pop_uint(4)
            self.count = buf.pop_uint(1)
            for i in range(self.count):
                key = TestParam(buf.pop_uint(1))
                length = buf.pop_uint(1)
                try:
                    expected_length = TestParam.defs[key]
                except Exception:
                    expected_length = length
                    logger.warning(f"{key.name} length not specified in uqt.")
                if expected_length != length:
                    logger.warning(
                        f"{key.name} not of expected length. Got {length}, expecting {expected_length}.\n Will use {length}"
                    )
                val = buf.pop_uint(length)
                self.params.append((key, val))
            if buf.remaining_size() != 0:
                logger.warning(
                    f'TestConfigSetReq: {buf.remaining_size()} unhandled remaining bytes: {buf.pop(-1).hex(".")}.'
                )
        except ValueError as e:
    ...
    def to_bytes(self, byteorder="little") -> bytes:
        ...
        for key, val in self.params:
            payload += key.to_bytes(1, "little")
            try:
                length = TestParam.defs[key]
            except Exception:
                raise ValueError(f"{key.name} length is not known.")
            payload += length.to_bytes(1, "little")
            payload += val.to_bytes(length, "little")
        return payload
...
class TestConfigGetResp:
    ...
    def __init__(self, payload: bytes):
        ...
        try:
            ...
            if buf.remaining_size() != 0:
                logger.warning(
                    f'TestConfigGetResp: {buf.remaining_size()} unhandled remaining bytes: {buf.pop(-1).hex(".")}.'
                )
        except ValueError as e:
...
class RxTestOutput:
    def __init__(self, payload: bytes):
        ...
        if self.buffer.remaining_size() != 0:
            logger.debug(
                f"RxTestOutput: {self.buffer.remaining_size()} unhandled remaining bytes."
            )
...
class PerRxTestOutput:
    ...
    def __init__(self, payload: bytes):
        ...
        if self.buffer.remaining_size() != 0:
            logger.debug(
                f"PerRxTestOutput: {self.buffer.remaining_size()} unhandled remaining bytes."
            )
...
class SrRxTestOutput:
    """
    This class is encapsulatin SR_RX_NTF data
    as described in Fira UCI Extension for testing.
    All values are in 'natural number' format.
    """

    def __init__(self, payload: bytes):
        self.gid = Gid.Test
        self.oid = OidTest.SrRx
        self.payload = payload
        self.buffer = Buffer(payload)
        self.status = Status(self.buffer.pop_uint(1))
        self.attempts = self.buffer.pop_uint(4)  # No. of RX attempts
        ...
        if self.attempts > 1:
            bitmap_length = self.attempts // 8
            if self.attempts % 8:
                bitmap_length += 1
            if bitmap_length > 38:
                bitmap_length = 38
            self.sts_detect_bitmap = self.buffer.pop_uint(
                bitmap_length
            )  # Sts detection Bitmap
        else:
            self.sts_detect_bitmap = 0  # Sts detection Bitmap
        if self.buffer.remaining_size() != 0:
            logger.debug(
                f"SrRxTestOutput: {self.buffer.remaining_size()} unhandled remaining bytes."
            )

    def __str__(self) -> str:
        return f"""# SR Rx Test Result:
        Status:                 {self.status.name} ({hex(self.status.value)})
        ATTEMPTS:               {self.attempts}
        ACQ_DETECT:             {self.acq_detect}
        ACQ_REJECT:             {self.acq_reject}
        ...
        STS_FOUND:              {self.sts_found}
        EOF:                    {self.eof}
        STS_DETECT_BITMAP:      {hex(self.sts_detect_bitmap)}"""
...
class LoopBackTestOutput:
    ...
    def __init__(self, payload: bytes):
        ...
        self.phr = self.buffer.pop_uint(2)  # Received PHR
        length = self.buffer.pop_uint(2)  # Length of following PSDU Data
        self.psdu_data = self.buffer.pop(length)  # PSDU Data

        if self.buffer.remaining_size() != 0:
            logger.debug(
                f"LoopBackTestOutput: {self.buffer.remaining_size()} unhandled remaining bytes."
            )
...
class UciMessage:
    ...
    def __init__(self, payload: bytes):
        ...
        b.pop_uint(1)  # RFU
        self.length = b.pop_uint(1)  # Payload Length
        self.payload = b.pop(self.length)
    ...
    def __str__(self) -> str:
        ...
        rts = (
            rts
            + f"{self.mt.name} (gid={hex(self.gid)}, oid={hex(self.oid)}, len={self.length}):\n"
        )
...
uci_codecs.update(
    {
        ...
        (MT.Command, Gid.Test, OidTest.SrRx): default_codec(
            "Start SR Rx", no_data=True
        ),
        (MT.Response, Gid.Test, OidTest.SrRx): default_codec(
            "Start SR Rx", status_only=True
        ),
        (MT.Notif, Gid.Test, OidTest.SrRx): SrRxTestOutput,
