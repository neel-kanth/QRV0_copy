...
from . import core
from . import fira
from .utils import DynIntEnum, Int8, Int32, Buffer
...
def klvs_from_bytes(defs, payload):
    ...
    for k in range(n):
        ...
        # kv_l_allowed = get_key_value_size(defs, key_name)

        # if kv_l != kv_l_allowed:
        # raise ValueError(f'bad calibration key {key_name}')
        ...
        res.append((key_name, fira.Status(k_s), kv_l, kv))
...
class Client_extension:
    ...
    def session_list_active(self):
        payload = self.command(fira.Gid.Qorvo, OidQorvo.ListActiveSessions, b"")
        return ActiveSessions(payload)

    def get_antenna_conf_fira(self, params):
        """params: liste of integers expected: chan_1, param_idx_1, chan_2, param_idx_2, ..."""
        params = [len(params) // 2] + params
        payload = self.command(
            fira.Gid.Qorvo, OidQorvo.FiraGetAntennaFlex, bytes(params)
        )
        return AntennaFlexParams(payload)

    def set_antenna_conf_fira(self, params):
        """params: liste of integers expected: chan_1, param_idx_1, value_1, chan_2, param_idx_2, value_2,..."""
        params = [len(params) // 3] + params
        payload = self.command(
            fira.Gid.Qorvo, OidQorvo.FiraSetAntennaFlex, bytes(params)
        )
        return fira.Status((int).from_bytes(payload, "little"))

    def test_tx_cw(self, switch_op):
        payload = self.command(
            fira.Gid.Qorvo, OidQorvo.TestTxCw, switch_op.to_bytes(1, "little")
        )
        return fira.Status((int).from_bytes(payload[0:1], "little"))

    def test_pll_lock(self, operation, timeout):
        if operation == 255:
            payload = self.command(fira.Gid.Qorvo, OidQorvo.TestPllLock, [])
        else:
            payload = operation.to_bytes(1, "little")
            payload += timeout.to_bytes(1, "little")
            payload = self.command(fira.Gid.Qorvo, OidQorvo.TestPllLock, payload)
    ...
    def test_tof(self, start_voltage, end_voltage, code_offset, timeout_delay):
        ...
        payload = self.command(fira.Gid.Qorvo, OidQorvo.TestTof, payload, timeout_delay)
        return fira.Status((int).from_bytes(payload[0:1], "little"))

    def test_rtc(self, time_out_ms):
        payload = self.command(
            fira.Gid.Qorvo, OidQorvo.TestRtc, time_out_ms.to_bytes(2, "little")
        )
        return fira.Status((int).from_bytes(payload[0:1], "little"))
    ...
    def test_mode_calibrations_set(self, params):
        ...
        payload = self.command(
            fira.Gid.ConfigManager, OidConfigManager.SetValDeprecated, payload
        )
        return (
            fira.Status((int).from_bytes(payload[0:1], "little")),
    ...
    def test_mode_calibrations_set_single_without_verif(self, key, value_len, value):
        ...
        status = fira.Status((int).from_bytes(payload[0:1], "little"))
        if status != fira.Status.Ok:
            return status
        else:
            return (
                fira.Status((int).from_bytes(payload[0:1], "little")),
    ...
    def test_mode_calibrations_get(self, params):
        ...
        payload = self.command(fira.Gid.ConfigManager, OidConfigManager.GetVal, payload)
        return (
            fira.Status((int).from_bytes(payload[0:1], "little")),
    ...
    def get_cal(self, params):
        ...
        get_cal = GetCalibration(
            self.command(fira.Gid.ConfigManager, OidConfigManager.GetVal, payload)
        )
        return get_cal.status, get_cal.n, get_cal.rtv

    def get_power_stats(self):
        payload = self.command(fira.Gid.Qorvo, OidQorvo.GetPowerStats, b"")
        b = Buffer(payload)
        rts = fira.Status(b.pop_uint(1))
        idle = b.pop_uint(4)
        tx = b.pop_uint(4)
        rx = b.pop_uint(4)
        wakeup_cnt = b.pop_uint(4)
        uptime = b.pop_uint(4)
        s1 = b.pop_uint(4)
        s3 = b.pop_uint(4)
        return (rts, idle, tx, rx, wakeup_cnt, uptime, s1, s3)

    def get_mem_stats(self):
        payload = self.command(fira.Gid.Qorvo, OidQorvo.GetMemStats, b"")
        b = Buffer(payload)
        rts = fira.Status(b.pop_uint(1))
        static_size = b.pop_int(4)
        heap_used = b.pop_uint(4)
        heap_peak = b.pop_uint(4)
        heap_size = b.pop_uint(4)
        nb_thread = b.pop_uint(1)
        stack = []
        for x in range(nb_thread):
            name_len = b.pop_uint(1)
            name = b.pop_str(name_len)
            stack_used = b.pop_uint(4)
            stack_peak = b.pop_uint(4)
            stack_size = b.pop_uint(4)
            stack.append((name, stack_used, stack_peak, stack_size))

        return (rts, static_size, heap_used, heap_peak, heap_size, stack)
    ...
    def config_manager_transaction_start(
        self, transaction_type=ConfigManagerTransactionType.Config
    ):
        payload = transaction_type.to_bytes(1, "little")
        payload = self.command(
            fira.Gid.ConfigManager, OidConfigManager.TransactionStart, payload
        )
        return fira.Status((int).from_bytes(payload[0:1], "little"))

    def config_manager_transaction_set_val(self, params):
        payload = kv_to_bytes(CalibrationParams.defs, params)
        payload = self.command(
            fira.Gid.ConfigManager, OidConfigManager.TransactionSetVal, payload
        )
        return (
            fira.Status((int).from_bytes(payload[0:1], "little")),
            struct.unpack("b{}s".format(payload[1]), payload[1:]),
    ...
    def config_manager_transaction_reset_val(self, params):
        payload = (len(params)).to_bytes(2, "little")
        for k in params:
            payload += len(k).to_bytes(1, "little")
            payload += k.encode("UTF-8")
        payload = self.command(
            fira.Gid.ConfigManager, OidConfigManager.TransactionResetVal, payload
        )
        return (
            fira.Status((int).from_bytes(payload[0:1], "little")),
            struct.unpack("b{}s".format(payload[1]), payload[1:]),
    ...
    def config_manager_transaction_end(self, end_type):
        payload = end_type.to_bytes(1, "little")
        payload = self.command(
            fira.Gid.ConfigManager, OidConfigManager.TransactionEnd, payload
        )
        return fira.Status((int).from_bytes(payload[0:1], "little"))

    def config_manager_lock_default_values(self):
        payload = self.command(
            fira.Gid.ConfigManager, OidConfigManager.LockDefaultValues, b""
        )
        return fira.Status((int).from_bytes(payload[0:1], "little"))
...
class GetCalibration:
    ...
    def decode_fira(self):
        ...
        b = self.buffer
        try:
            self.status = fira.Status(b.pop_uint(1))
            self.n = b.pop_uint(2)
            self.rtv = []
            for k in range(self.n):
                ...
                if length == 0:
                    continue
                k = b.pop(length).decode()
                s = fira.Status(b.pop_uint(1))
                length = b.pop_uint(1)
                v = b.pop(length)
                self.rtv.append((k, s, length, v))
...
fira.uci_codecs.update(
    {
        (
            core.MT.Response,
            fira.Gid.ConfigManager,
            OidConfigManager.GetVal,
        ): GetCalibration,
    }
)
...
fira.notification_default_handlers.update(
    {
        (fira.Gid.Ranging, fira.OidRanging.Start): lambda x: print(RangingData(x)),
        (fira.Gid.Test, fira.OidTest.Loopback): lambda x: print(LoopBackTestOutput(x)),
        (fira.Gid.Test, fira.OidTest.PeriodicTx): lambda x: print(
            PeriodicTxTestOutput(x)
        ),
        (fira.Gid.Test, fira.OidTest.SsTwr): lambda x: print(TwrTestOutput(x)),
        (fira.Gid.Test, fira.OidTest.Rx): lambda x: print(RxTestOutput(x)),
        (fira.Gid.Test, fira.OidTest.PerRx): lambda x: print(PerRxTestOutput(x)),
    }
)

fira.notification_default_handlers.update(
    {
        (fira.Gid.Qorvo, OidQorvo.TestDiag): lambda x: print(RangingDiagData(x)),
        (fira.Gid.Qorvo, OidQorvo.DeviceBootNtf): lambda x: print(DeviceBoot(x)),
        (fira.Gid.Qorvo, OidQorvo.PSDUSReport): lambda x: print(RangingPSDUSReport(x)),
        (fira.Gid.Qorvo, OidQorvo.SessionDataXferStatusNtf): lambda x: print(
            SessionDataTransferStatus(x)
        ),
        (fira.Gid.Qorvo, OidQorvo.TestDebug): lambda x: print(TestDebugData(x)),
        (fira.Gid.Qorvo, OidQorvo.TestPllLock): lambda x: print(PllLockTestOutput(x)),
        (fira.Gid.Qorvo, OidQorvo.TestRtc): lambda x: print(RtcTestOutput(x)),
        (fira.Gid.Qorvo, OidQorvo.TestTof): lambda x: print(TofTestOutput(x)),
    }
)
