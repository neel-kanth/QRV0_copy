...
from uci.utils import Uint8, Int8, Int16, Uint16, Uint24, Uint32, Uint64
    ...
    while True:
        try:
            ...
            try:
                ...
                if args.value[0].lower().startswith("0x"):
                    # Remove the "0x" prefix
                    stripped = args.value[0][2:]
                    if stripped:
                        v = stripped.lstrip("0")
                        v = v if v else "00"
                        v = f"{cal_params[args.key].__name__}('{v}')"
                        value = eval(v)
