...
from uci import OidTest, OidQorvo, TestModeSessionId
from uci import TestDebugData, LoopBackTestOutput
from uci import NotImplementedData
from uqt_utils.utils import test_profile_keys, get_test_profile, uqt_errno, str2bytes
...
def main():
    parser = argparse.ArgumentParser(
        description="run Fira's Loopback test.",
    ...
    while True:
        try:
            ...
            print(f'{"init TestSession:":<25}{rts.name}')
            if rts != Status.Ok:
            ...
            rts = client.test_loopback(p_tx)
            print(f'{"Start Loopback test:":<25}{rts.name}')
            if rts != Status.Ok:
                print(f"test_loopback failed: {rts.name} ({rts}).")
                break
