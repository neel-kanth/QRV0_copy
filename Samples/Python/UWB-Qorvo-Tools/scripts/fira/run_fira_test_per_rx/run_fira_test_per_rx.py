...
from uci import Client, App, Gid, SessionType, TestParam, Status, UciComError
from uci import OidTest, TestModeSessionId
from uci import PerRxTestOutput
...
default_psdu = "00.00.00.00.00.00"
...
def main():
    parser = argparse.ArgumentParser(
        description="run Fira's PER_RX test.",
    ...
    parser.add_argument(
        ...
        type=int,
        default=9,
        help="This value configures the preamble code index 9 to 24 are allowed for BPRF"
        " and 25 to 32 for HPRF. (default: %(default)s)",
    )
    parser.add_argument(
        "--sfd-id",
        type=int,
        choices=[0, 1, 2, 3, 4],
        default=2,
        help="Start of frame delimiter, allowed values are 0, 1, 2, 3, 4 (default: %(default)s)",
    )
    parser.add_argument(
        "--rframe-config",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=2000,
        help="Gap between start of one packet to the next in   s. (default: %(default)s)",
    )
    parser.add_argument(
        "--timeout",
        type=int,
        default=3,
        help="Time to receive first package. (default: %(default)s)",
    )
    parser.add_argument(
        "--en-diag",
        action="store_true",
    ...
    def per_rx_test_callback(payload):
        nonlocal action_completed
        action_completed = True
        print(PerRxTestOutput(payload))

    notif_handlers = {(Gid.Test, OidTest.PerRx): per_rx_test_callback}
        ...
        try:
            ...
            rts, rtv = client.test_config_set(
                ...
                [
                    ...
                    (TestParam.TStart, 450),
                    (TestParam.TWin, 750),
                    (TestParam.RandomizePsdu, args.randomized_psdu),
                    (TestParam.PhrRangingBit, 0),
            ...
            app_configs = [
                ...
                (App.DeviceMacAddress, [0x00, 0x8C]),
            ...
            rts = client.test_per_rx(p_tx)
            print(f'{"Start Rx PER test:":<25}{rts.name}')
            if rts != Status.Ok:
                print(f"test_per_rx failed: {rts.name} ({rts}).")
                break
