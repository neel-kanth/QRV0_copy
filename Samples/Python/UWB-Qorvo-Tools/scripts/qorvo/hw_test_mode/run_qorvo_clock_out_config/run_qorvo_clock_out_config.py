...
def main():
    ...
    parser.add_argument(
        "--pin-id",
        type=int,
        default=PinId.PinExton,
        help=f"Pin ID (EXTON: {PinId.PinExton})",
    )
    parser.add_argument(
        "--clock-id",
        type=int,
        help="Clock ID (depends on SOC)",
    )
    ...
    try:
        with Client(
            ...
            rts = client.clock_out_config(
                clock_id,
                PinId(args.pin_id),
                args.flags,
            )
            assert rts == Status.Ok, f"clock_out_config failed: {rts.name} ({rts})"
